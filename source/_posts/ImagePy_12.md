...
# 画布Canvas运行机理
总体看一下它的调用代码（这里是查看单独调用该画布时的用法，该画布也可以直接用在ImagePy中）：
```python
...
canvas.set_img(msk)
```
这一步是设置Canvas要显示的图像，因为这个例子是有点特别，是用于标注，所以此时Canvas要显示的图像是掩膜msk。
...
这里也提前说明，不带下划线的属性（包括self.lut、self.rg和self.cn）都是与Canvas要显示的图像相关，而带下划线的属性（包括self._lut、self._rg和self._cn）都是与背景图相关。因为这里掩膜是要显示的图像，所以给它配套的查找表设置为自定义的查找表。

## 配置Canvas：设置图像通道
```python
canvas.set_cn(0)
```
这一步是设置图像通道，这里因为只传递了0，所以第二个参数用的是False，即这里不是针对背景图设置的，而是针对要显示的图，即self.cn设为0。这里有两点需要注意：首先因为是cn，不带下划线，说明是对要显示的图的设置；再者一定要设置为0，这是通道标识，因为掩膜msk是一个灰度值，所以只有一个通道，那么它的标识就是0。
...
canvas.set_mode('msk')
```
这里将模式设为msk。因为如前所述，该例子是为了标注。关于mode的详述，见本文下面的set_mode()方法。
...
conbox =  [0, 0, 512, 512]
```
通过设置图像，给Canvas的img属性赋值，同时根据图像尺寸得到了Canvas的conbox和oribox。
...
self._cn = 0
```
这里传入的cn是通道标识，如果要显示的图像只有一个通道，一定要设置为0；如果有多个通道，比如RGB，若想显示彩图，则设置为RGB；若只想显示其中一个通道，则设置通道标识为0或1或2。
...
```
有多种混合模式：
（1）set模式：默认模式，直接将图像设为要显示的图像
（2）min模式：逐个对比背景图和前景图两张图像中的像素，选择数值小的像素保存进要显示的图像
（3）max模式：逐个对比背景图和前景图两张图像中的像素，选择数值大的像素保存进要显示的图像
（4）msk模式：根据掩膜将背景图和前景图进行混合，在掩膜msk不为0的地方，将背景图设为0；在msk为0的地方，保留原值（因为对msk作了逻辑非操作，然后下面主元素相乘时，True为1，False为0）。然后将两图进行相加合成。
（5）小数模式：mode可以是一个小数，即将背景图与1-mode相乘，前景图与mode相乘，然后两者相加合成。
...
可以看出mix_img()方法调用了两次，分别是为了设置背景图和设置要显示的图像。两者的顺序不能调换，因为第一次的调用返回的self.outrgb是作为背景图在第二次调用时进行图像混合。
...
（5）小数模式：mode可以是一个小数，即将out与1-mode相乘，img与mode相乘，然后两者相加合成。
...
def mix_img(img, m, o, shp, buf, rgb, byt, rg=(0,255), lut=None, cns=0, mode='set'):
```
第一个是img图像，从前面的调用可知，可以传入要显示的图像或背景图；
第一个是m，旋转缩放矩阵；
第三个是shp，即最终要绘图的区域大小；
